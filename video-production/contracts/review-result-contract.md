...
- Natural-boundary review distinguishes no added outline, preserved approved source outline and necessary runtime outline; it checks complete visible contours, neighbor fragments and semantic map continuity. Bounded recovery includes pre-entrance and cutout exposure with no residual stamp/text ghost. Record static extremes and intermediate exposure in existing `poster_readiness`/`editability` findings; do not create a new Review schema.
- Static motion feasibility is not perceptual motion approval. The actual-media owner remains existing `production_qa` via `qa-retry.md`, which reviews declared relative motion and ending intent against final encoded playback. Unknown viewing evidence stays unknown, never PASS based on transform parameters or pixel differences.
