...
input:
  ...
  video_adapter: {platform: string | null, access_route: string | null, model_id: string | null, model_version: string | null, target_resolution: string | null, aspect_ratios: [string], duration_options_seconds: [number], reference_modes: [string], native_audio_strategy: native_speech | silent | unknown, evidence_refs: [string]}
...
- For a video project, the Controller requests the minimum `video_adapter` record at project start. Unknown or platform-unverified fields stay explicit; Scene/Clip/Prompt planning must not silently substitute official model capabilities for the actual execution surface.
