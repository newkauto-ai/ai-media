...
For new Review Result v2.1 runs, the same compact evaluator call also applies [Prompt Story Function Conformance](prompt-story-function-review.md) to the explicit upstream role assignment. Do not launch a second reviewer, count actions, impose universal timing, or treat a keyword match as story evidence. Persist the result through [Review Result v2.1](../contracts/review-result-contract.md); legacy Feasibility v1.1 fixtures remain read-only compatibility inputs.
