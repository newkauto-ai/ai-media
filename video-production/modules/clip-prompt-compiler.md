...
Keep the montage truthful across calls: the previous result remains the next input, and each independent prompt receives the required state reference. Preserve the approved adaptive sequence without encoding or inventing any fixed shot ratio, default order, or mandatory alternation. Project-grounded relationships such as “the real exterior and the crafted image echo one another” are allowed only when both elements are confirmed by the current project; do not automatically inject Jiangnan, Hanfu, embroidery, tea, or a female subject.
