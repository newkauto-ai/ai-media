...
Adaptive route fields belong only to contract `1.2+`; contract `1.0` and `1.1` remain read-only compatibility inputs and must not be used to force `flat_auto` for a new managed render. For `1.2+`, only a source classified `simple` with bounded detected text may resolve to `flat_auto`. A `moderate` or `complex` flat source resolves to required route `structured_semantic` and returns `human_review` until explicit layers are supplied, even if the caller requested `flat_auto`. The report distinguishes the requested route from the required route and lists the blocking evidence.

For manual adaptive flat timing, preflight compares the actual component-schedule end with `draw_duration_seconds`. Any overflow returns `human_review`; extending the encoded file after the fact is not a valid routing or pacing fix.
...
Contract `1.3` adds upstream production planning without adding a second Renderer. `whiteboard-production-cli.ps1` maps a registry-`ready` Profile to a deterministic `whiteboard_style_slice`; it records accept/transform/drop/block decisions and the source/slice hashes. The Renderer receives only already-authored assets and the immutable Job. It does not open, parse, or reinterpret a Style Profile. Authored people or other character layers must declare `semantic_kind: character`, `stroke_order_policy: head_first`, and an output-space `head_bbox`; both their line and fill passes complete the head region before the remainder, without anatomical guessing.

New character Jobs use `stroke_order_policy: head_body_upper_arms_forearms_hands_thighs_lower_legs_feet` and exactly eight checksum-bound authored part masks in this fixed order: `head`, `body`, `upper_arms`, `forearms`, `hands`, `thighs`, `lower_legs`, `feet`. Animals map these to head, torso, upper/lower forelimbs, front paws, upper/lower hindlimbs, and hind paws. Every part also binds separate `outline_mask_rgba` and `detail_mask_rgba`; its line pass completes the outline before any authored interior details, while fill preserves the same anatomical order. These masks are authoritative semantic evidence—the Renderer does not infer anatomy or interior meaning from connectivity or position. Missing, reordered, overlapping, out-of-part, geometry-mismatched, or non-exhaustive masks are rejected. `head_body_hands_feet` and `head_first` plus `head_bbox` remain compatibility input only and must not be authored for new Jobs.
