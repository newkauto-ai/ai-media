...
For selected `oriental_pastoral_cinematic_lifestyle` craft montage, use the same Storyboard Plan and `previsualization_storyboard` Review Result. Confirm that the representative Panels include `atmosphere_shot`, `process_detail_shot`, and `progression_shot` when all three functions are present in the approved plan; do not manufacture a missing function merely to fill a quota. Check that an atmosphere Panel has a named breathing, regional, emotional, contrast, visual-echo, or aftertaste function; a process-detail Panel shows a credible bounded action without claiming a full tutorial loop; and a progression Panel binds prior/current material states with a visible `completion_delta`. Five or six consecutive same-function craft macros, “beauty + environment + finished product” without credible making evidence, portrait-studio staging, tourism/product-promo drift, magic growth, full-frame morph, or plastic 3D craft output is `must_fix`. The suggested two-process/one-atmosphere rhythm is a soft planning tendency, not a Gate or automatic failure count.
