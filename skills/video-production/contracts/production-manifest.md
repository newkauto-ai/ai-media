...
- Every formal VOX local-assembly Shot records `poster_shot_id`, `source_beat_id`, `shot_role`, `poster_archetype`, `adjacent_shot_difference`, a non-empty `stable_poster_state`, `poster_spec`, and `motion_plan`. The Poster Shot inherits its ADP Beat's semantic function; one Beat may project to multiple Poster Shots without rewriting Script/ADP semantics.
- `poster_spec` records visual hierarchy, primary/secondary elements, background/foreground systems, reading path, text safe area, `critical_text_owner`, `generate_text_in_image`, stable-poster-state ref, and current Poster Readiness Review ref. Critical VOX names, places, dates, numbers, evidence, quotes, statistics, CTA, and long titles default to `critical_text_owner: remotion` and `generate_text_in_image: false`; exceptions are explicit and reviewable.
...
- An unresolved Poster Readiness `must_fix` blocks motion compilation for the affected Poster Shot. Missing/stale review evidence maps through existing Review Result/Controller rules and does not consume retry. No Poster Gate, Poster State Machine, Poster Retry Ledger, VOX Manifest, or separate approval state is created.
