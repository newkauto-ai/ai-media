...
For VOX natural-layer micro-animation, existing asset notes, layer geometry/anchors, motion timing/phases and Review refs may describe contour/fused limits, retained source outline or optional runtime outline, required pre-entrance/occlusion recovery, shared versus relative transforms and continuous-to-last-frame versus intentional-hold intent. These are descriptions inside current records, not new required fields, an enum migration, a parallel checklist or a new state owner. Static exposure feasibility and actual-media perceptual QA remain distinct.

## VOX v2.2 compatible production projection

- Keep each Shot's `source_beat_id` scalar. Multiple Shot rows may reference one exact Poster asset revision across Beats; one Beat may contain multiple Shot rows with different Posters. `poster_shot_id` identifies a use, not the source image. Each use retains its own absolute interval, layout, text visibility, crop and applicable Review; a logical boundary does not mandate a visible cut or generated image.
- Before new design requests, the existing `local_assembly_plan.poster_spec.production_design` may carry `layout_intent` (reading path, text region, independent groups), `base_asset_id`, `overlay_asset_ids`, exact `layout`, `renderer_ref`, and later `preview_asset_ref` with input fingerprint/checksum, `review_refs`, detailed `motion_preplan`, `output_geometry` and `render_input_hash`. Existing `assets` carry `design_role`, `media_ref` (absolute path/revision/SHA-256), `used_by_poster_shots`, and for new requests `request_evidence` with bounded reuse search, real gap and authorization reference. These are compatible fields in existing owners, not a second Manifest or Gate.
- Base suitability evidence allows independent overlay design only. The complete Preview is composed locally from exact Base/Overlay files and layout; the final composite is never redrawn by an image model. Native independent layers are reused. Critical design text must have a verified typography reference, and a route texture used for progressive reveal must have registered path/mask geometry. Effective visible subject pixels, not atlas canvas size, govern reuse and planned scale.
- `skills/video-production/scripts/validate-vox-production.ps1` is the read-only mechanical preflight for request, Preview, motion preplan, formal render and intake actions. It checks the existing Shot/Asset/Review records and exact media. Passing only permits the named action; fixture, candidate, naked render and base-only evidence do not become formal Poster or Motion PASS. Actual image design, glyph/fact quality and continuous viewing remain human/visual Review responsibilities.
- Formal render binds the confirmed Master Audio revision/checksum and narration interval in its input fingerprint. Intake recomputes that fingerprint, binds the exact encoded output checksum, and requires existing technical and continuous-viewing Reviews in `qa.results`; a bare nonempty viewing reference cannot approve an output.
