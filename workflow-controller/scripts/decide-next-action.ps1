...
if ($null -ne $reviewResult) {
    ...
    if ([string]$reviewResult.schema_version -ne '2.1' -or [string]$reviewResult.gate -ne 'pre_generation_prompt') {
        throw 'Only Review Result v2.1 pre_generation_prompt is implemented.'
    }
    ...
    foreach ($field in @('frozen_script', 'audiovisual_direction_package', 'production_manifest', 'video_prompt_spec', 'executable_prompt')) {
    ...
    if ($mappedVerdict -eq 'pass' -and ([string]$reviewResult.decision.verdict -ne 'PASS' -or [string]$reviewResult.decision.generation_gate_recommendation -ne 'eligible_for_separate_cost_gate')) {
        throw 'Only a Review PASS may recommend entry to the separate Cost Gate.'
    }
...
$requiresResearch = (($unit.stage -ne 'pre_generation_prompt_review') -and $assessment.requires_external_evidence) -or (($unit.topic_type -in @('historical', 'science', 'health')) -and $unit.stage -in @('topic_selection', 'research'))
...
elseif ($assessment.verdict -eq 'blocked') {
    $decision.state = 'blocked'
    $decision.selected_capability = if ($unit.stage -eq 'pre_generation_prompt_review') { 'video-production' } else { $assessment.proposed_capability }
...
elseif ($assessment.verdict -eq 'retry') {
    if ([int]$unit.retry_count -ge [int]$unit.max_retries) {
        ...
        elseif ($unit.stage -in @('pre_generation_prompt_review', 'production_qa')) { $decision.selected_capability = 'video-production' }
