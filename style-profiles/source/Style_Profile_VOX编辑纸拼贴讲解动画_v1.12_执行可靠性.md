...
- 一个 Beat 可包含多个 Shot/Poster；同一 Poster 源可被多个 Beat 的 Shot 复用。每次使用保留单值 `source_beat_id`、绝对时段、准确资产版本、布局、文字可见性与适用 Review。Beat 边界不强迫可见切镜，也不因字幕分句或固定秒数自动建图。
- 新图前做有界复用检索，记录范围、候选版本、适用/拒绝理由与真实缺口。出图前定整体阅读路径、文字区域、独立组、晚显信息、分离/纹理与取景余量。计划候选可依据上游基线和具体授权制作，不要求尚不存在的完整 Poster 已批准。
- 新 `production_reconstructable` 默认先复用/取得干净场景 Base 并确认适用；再复用/制作单件优先的透明设计覆盖层；最后用同一素材和布局本地确定性合成完整 Poster Preview。Base 认可只许可继续覆盖层，不能冒充完整 Poster 认可；终稿不交图像模型重画。Hero 与已批准整图可依有效证据保留。已独立的层直接复用，不压平后再拆。
- 完整 Poster 通过后才按真实几何、旁白和注意力目标做详细运动预案；该预案决定最小拆解及隐藏区域恢复。静态终态、入场前、交接、中间与极限检查有效且未修 `must_fix` 清零后才正式编排/渲染 Motion。预案不是 Motion 执行。
- 关键设计字/引文可用经字形、事实、简体、Alpha、尺寸、权利核验的独立 SVG/PNG；候选不能写成 `verified_typography`。路线纹理 PNG 不等于可动画路径，沿路推进须有同坐标配准的路径或遮罩。Remotion 管布局、遮罩和时间。无需逐字/沿路效果时可整组使用，不强制拆字/拆节点。
- 关键标题、人物、复杂道具、大路线和主要框线默认单件透明资产。Atlas 只用于小、简单、非关键元素且提取后可见主体像素满足当前最大显示比例；已合格提取件可继续复用。画布尺寸与简单放大不是有效分辨率证据。底图、覆盖层及最终编码共同审查材料密度和时间稳定性。
- 受管执行由现有 Manifest/Shot/Asset/Review/Execution State 持有。Review v2.2 对 VOX 必检使用明确 `check_result` 和 `must_fix` 解决证据；legacy assessment 与旧 Review 只作迁移诊断。请求、Preview、正式 render 和 intake 用同一只读机械预检，媒体质量和用户认可仍由真实 Review 判定。

以下结构块是本节可机械核对的规则源；`normalized.poster_first_planning.execution_reliability` 必须逐项相同。审美判断仍以上文与真实 Review 为准。

```json
{
  "planning": "bounded_reuse_search_then_joint_beat_shot_poster_use_and_layout_intent_before_new_image_request",
  "production_default": "approved_clean_base_then_independent_transparent_design_overlays_then_deterministic_complete_preview_without_model_redraw",
  "candidate_rule": "planned_shot_and_upstream_baseline_permit_authorized_design_candidate_without_complete_poster_approval",
  "approval_scope": "base_suitability_only_permits_overlay_design_complete_preview_review_precedes_detailed_motion_preplan",
  "motion_order": "complete_poster_then_detailed_preplan_then_minimum_decomposition_and_exposure_recovery_then_static_key_states_then_formal_motion",
  "native_layer_reuse": "reuse_independent_overlays_without_flattening_and_reextracting",
  "typography": "candidate_glyph_fact_simplified_chinese_alpha_size_rights_check_before_verified_svg_or_png_remotion_owns_layout_and_timing",
  "route": "texture_png_needs_registered_path_or_mask_for_progressive_reveal_whole_group_use_needs_no_path",
  "asset_size": "critical_assets_single_item_by_default_atlas_only_if_extracted_visible_content_pixels_satisfy_max_use",
  "managed_review": "vox_review_v2_2_explicit_check_result_unresolved_must_fix_or_unknown_blocks_legacy_assessment_diagnostic_only",
  "entrypoints": "same_read_only_preflight_for_request_preview_motion_render_and_intake"
}
```
