...
> **升级来源**：田园风 v1.0–v1.1 + `田园风_style_profile_升级_short_spec_插件交接版.md`  
> **核心定位**：真人东方田园电影美学 + 诗意生活 + 可信局部工艺动作 + 成果蒙太奇递进 + 地域 / 民俗 / 非遗文化可扩展能力  
> **适用范围扩展**：田园生活、古村生活、农事、采摘、传统饮食、茶事、园艺、非遗民俗、传统工艺、手作、节气与乡土文化展示  
> **v1.2 核心升级**：在 v1.1 可复用 Style System 上增加“意境呼吸 + 局部工艺 + 成果递进”的诗意蒙太奇；保留工艺事实与材料状态连续性，但不要求单镜完成精确教学闭环。
...
当项目事实或脚本表明题材属于轻工艺、非遗、民俗制作、手作、轻料理或轻制造时，Scene/Shot planning 为相关镜头增加一个可选 `pastoral_craft_shot_type`：
...
pastoral_craft_shot_type:
  allowed:
    ...
    - process_detail_shot
    - progression_shot
  non_craft_default: null

pastoral_craft_montage:
...
- `atmosphere_shot`（意境 / 呼吸镜头）：建立地域、季节、空间、光影、人物感受或视觉互文。它必须承担转气、升格、对照、回声或余韵等明确叙事功能，不是可随意删除的补空镜。
- `process_detail_shot`（工艺镜头）：让观众明确感到人物正在做。优先指尖整理、工具靠近、接触、轻落、稳定触碰、衣袖掠过与材料轻微位移等可信短动作；不要求一个 Shot 复现复杂工序闭环。
- `progression_shot`（成果递进镜头）：通过同一对象在后续阶段的可核验完成度差异说明进度。必须记录前态引用、当前态与具体 `completion_delta`；禁止魔法生长、整图 morph 或无来源的瞬间完成。

江南建筑、水乡烟雨、汉服女性、刺绣、茶事等只能在当前项目事实、Script、ADP 或 Project Visual Bible 已确认时使用；它们是可用题材和 Preset，不是通用 Style Core 的硬编码默认值。
...
继承 v1.1 面向非遗、手作、传统工艺的材料状态真实性，并在 v1.2 明确：完整状态链可以分布在多个镜头中，不要求任何单镜承担全部动作与工序逻辑。
...
这三项可以由 `process_detail_shot → progression_shot` 或多镜头组合共同证明。单个 `process_detail_shot` 只需有可信起点、局部动作与可观察接触/轻微反馈；复杂针法、编织闭环、机械重复或完整料理步骤不能为了“讲清楚”而强塞进一个生成镜头。

`progression_shot` 的后态必须与前态和事实步骤相容。允许用 match cut 或 visual echo 连接相邻完成阶段，但不得把跨阶段变化伪装成连续物理生成，也不得用整图 morph 代替工艺过程。
...
## 工艺 / 民俗
不要跳过关键状态变化；不要伪造工艺流程；不要随意混用工具；不要改写仪式顺序；不要为唯美牺牲工艺可读性；不要将地域文化符号机械堆满每个镜头；不要默认输出精确教程片；不要让单镜强行闭合复杂机械动作；不要用连续微距替代人物、意境与成果递进；不要用魔法生长、整图 morph、3D 塑料化成品或无证据瞬间完成表达进度。

## 叙事退化
不要退化为“美女 + 环境 + 成品展示”、影楼古风写真、纯旅游宣传片或纯产品广告。人物不持续直视镜头，不用摆拍姿势和过度滤镜/古偶化代替真实在场、局部动作与材料变化。
...
production_modules:
  ...
  prompt_compilation:
    ...
    require_pastoral_craft_shot_type: when_craft_montage_applies
    process_detail_clause: 可信局部动作，不要求单镜完成教学闭环
    progression_clause: 用前后完成阶段与具体 completion_delta 表达推进
    atmosphere_clause: 呼吸镜头必须承担地域、情绪、对照、互文或余韵功能
    match_cut_or_visual_echo: allowed_when_grounded_by_project_states
    prohibit_magic_growth_or_full_frame_morph: true
  ...
  storyboard_planning:
    ...
    output_fields:
      - pastoral_craft_shot_type
      - breathing_shot_note
      - prior_process_state_ref
      - current_process_state
      - completion_delta
      - match_cut_or_visual_echo
...
normalized_from_source: Style_Profile_田园风_v1.2_诗意工艺蒙太奇.md
...
style_profile:
  ...
  pre_content_modules:
    ...
    story_direction:
      authority: medium
      value: 环境建立 → 人物进入 → 局部动作 → 意境呼吸 → 成果递进 → 情绪回落 → 环境余韵
  ...
  audiovisual_modules:
    ...
    shot_design:
      ...
      craft_shot_types: [atmosphere_shot, process_detail_shot, progression_shot]

    process_detail:
      authority: hard
      value: 只要求可信且有美感的局部动作；不要求单镜完成复杂教学闭环，完整状态链可跨镜头成立
    ...
    content_guardrails:
      authority: hard
      avoid: [precise_tutorial_reconstruction, beauty_environment_product_shell, portrait_studio_staging, tourism_promo, product_only_ad, magic_growth, full_frame_morph, plastic_3d_craft_result]
  ...
  production_modules:
    ...
    prompt_compilation:
      ...
      require_sound_direction: true
      require_pastoral_craft_shot_type: when_craft_montage_applies
      require_breathing_shot_note: when_atmosphere_shot
      require_completion_delta: when_progression_shot
      allow_match_cut_or_visual_echo: when_grounded_by_project_states
      prohibit_magic_growth_or_full_frame_morph: true
  ...
  provenance:
    source_documents:
      ...
      - file: Style_Profile_田园风_v1.1_中文解析.md
        version: v1.1-zh
        source_type: previous_profile
        evidence: 江南采茶、制茶、泡茶、品茗项目多轮 LookDev、视频生成、镜头节奏重构与最终成片经验，以及面向田园生活、非遗民俗与工艺品制作的扩展审查

      - file: Style_Profile_田园风_v1.2_诗意工艺蒙太奇.md
        version: v1.2-zh
        source_type: short_spec_incremental_upgrade
        evidence: 田园风_style_profile_升级_short_spec_插件交接版.md
```

---

# 31. v1.1 → v1.2 Migration Notes
...
- 新增 source：`Style_Profile_田园风_v1.2_诗意工艺蒙太奇.md`
- 新增 normalized：`oriental_pastoral_cinematic_lifestyle.v1.2-zh.json`
- 不覆盖 v1.0 / v1.1 source 与 normalized 文件
- registry 当前条目更新至 v1.2；状态继续保持 `pending_review`，直到真实 LookDev / Pilot 获得人工接受
- 旧版本保留为历史，避免破坏已有项目引用
...
> **田园风 = 以真人东方自然主义电影为视觉基础，通过任务驱动摄影与真实人物在场，把意境呼吸镜头、可信局部工艺动作和可追溯的成果递进组织成诗意蒙太奇，在不伪造精确教学闭环的前提下，呈现田园生活、乡土文化、非遗民俗和传统工艺的真实、通透、有人情味且有触感的影像。**
