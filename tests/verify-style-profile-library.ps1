...
Assert-True (($pastoralAV.editing_rhythm.suggested_pattern -join '|') -eq 'process_detail_shot|process_detail_shot|atmosphere_shot' -and -not $pastoralAV.editing_rhythm.hard_quota) 'Pastoral craft rhythm must be a two-process/one-atmosphere soft tendency.'
...
Assert-True (-not $pastoralProduction.storyboard_planning.rhythm_is_hard_quota) 'Pastoral storyboard rhythm must not become a hard quota.'
