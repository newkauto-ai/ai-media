...
Assert-True ($pastoralEntry[0].normalized_version -eq 'v1.2-zh') 'Pastoral registry must route the stable ID to v1.2-zh.'
Assert-True ($pastoralEntry[0].source_file -eq 'Style_Profile_田园风_v1.2_诗意工艺蒙太奇.md') 'Pastoral registry must route to the v1.2 source profile.'
Assert-True ($pastoralProfile.style_profile.version -eq 'v1.2-zh') 'Pastoral normalized profile must declare v1.2-zh.'
...
Assert-True (Test-Path -LiteralPath (Join-Path $normalizedDir 'oriental_pastoral_cinematic_lifestyle.v1.1-zh.json')) 'Pastoral v1.1 normalized history must be retained.'
...
Assert-True ((@($pastoralAV.visual_domains.values | Sort-Object) -join ',') -eq 'craft_process,folk_culture,food_tea,pastoral_lifestyle') 'Pastoral v1.2 must preserve the four approved visual domains.'
...
Assert-True ($pastoralAV.shot_architecture.value -match '环境负责世界') 'Pastoral v1.2 must preserve functional shot architecture.'
...
Assert-True ($pastoralAV.process_continuity.may_span_multiple_shots -eq $true) 'Pastoral v1.2 process continuity must allow truthful state progression across shots.'
Assert-True ((@($pastoralAV.shot_design.craft_shot_types) -join '|') -eq 'atmosphere_shot|process_detail_shot|progression_shot') 'Pastoral v1.2 must expose the three craft montage shot types.'
Assert-True ($pastoralAV.process_detail.single_shot_tutorial_closure_required -eq $false) 'Pastoral process detail must not require a single-shot tutorial closure.'
Assert-True ($pastoralAV.editing_rhythm.decision_mode -eq 'adaptive_sequence_design' -and $null -eq $pastoralAV.editing_rhythm.fixed_pattern -and $null -eq $pastoralAV.editing_rhythm.fixed_ratio) 'Pastoral craft rhythm must be adaptive without a fixed pattern or ratio.'
Assert-True ((@($pastoralAV.editing_rhythm.decision_inputs) -join '|') -eq 'narrative_intent|process_complexity|material_state_change|information_density|human_space_relationship|music_emotional_arc|breathing_need|visible_completion_delta') 'Pastoral adaptive rhythm inputs are incomplete.'
Assert-True ($pastoralAV.content_guardrails.avoid -contains 'beauty_environment_product_shell' -and $pastoralAV.content_guardrails.avoid -contains 'full_frame_morph') 'Pastoral v1.2 content guardrails are incomplete.'
...
Assert-True ($pastoralProduction.prompt_compilation.require_pastoral_craft_shot_type -eq 'when_craft_montage_applies') 'Pastoral prompt compilation must project the craft montage shot type when applicable.'
Assert-True ($pastoralProduction.prompt_compilation.require_completion_delta -eq 'when_progression_shot') 'Pastoral progression prompts must require a completion delta.'
Assert-True ($pastoralProduction.storyboard_planning.rhythm_decision_mode -eq 'adaptive_sequence_design' -and $null -eq $pastoralProduction.storyboard_planning.fixed_pattern -and $null -eq $pastoralProduction.storyboard_planning.fixed_ratio) 'Pastoral storyboard rhythm must stay adaptive without a fixed pattern or ratio.'
...
Assert-True (($pastoralProfile.style_profile.provenance.source_documents | Where-Object version -eq 'v1.2-zh').Count -eq 1) 'Pastoral provenance must record the v1.2 poetic craft montage upgrade.'
